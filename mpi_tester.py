...
# Stolen from Psydac's repo https://github.com/pyccel/psydac
import pytest
    ...
    def dec(func):
        ...
        def wrapped(size, *args):
            if MPI.COMM_WORLD.size < size:
                pytest.skip(
                    "Test skipped because world is too small. Include the test with mpirun -n %d"
                    % (size)
                )
    ...
    def _end_capture_and_exit(self, code):
        ...
        with Rotator(self.comm):
            if self.comm.rank != 0:
                ...
                self.oldstderr.write(
                    "=" * 32
                    + " Rank %d / %d " % (self.comm.rank, self.comm.size)
                    + "=" * 32
                )
                self.oldstderr.write("\n")
